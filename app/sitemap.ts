...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [{ data: stalls, error: stallsError }, { data: posts, error: postsError }] = await Promise.all([
  ...
  // Products have no page of their own -- every active product lives
  // inside its stall's page (app/stalls/[slug]/page.tsx), not at a
  // separate URL, and the products table has no slug/dedicated route to
  // list individually. A stall's entry here already covers discoverability
  // of everything active on it, which is as close as this app's actual
  // routing gets to "every active product."
  ...
  return [...staticEntries, ...stallEntries, ...magazineEntries];
}
